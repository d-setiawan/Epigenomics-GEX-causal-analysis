#!/usr/bin/env python3
"""Joint preprocessing for scGLUE with RNA + all histone marks."""
...
from preprocess_pilot_scglue import (
    build_chrom_adata,
    infer_repo_root,
    pick_rna_h5,
    preprocess_chrom_for_scglue,
    preprocess_rna,
    resolve_path,
)
...
def main() -> int:
    p = argparse.ArgumentParser(description="Joint preprocessing for scGLUE")
    ...
    p.add_argument("--out-dir", default=None, help="Default: integration/outputs/scglue/joint/<RUN_ID>/preprocess")
    ...
    p.add_argument("--rna-hvg-flavor", default="seurat_v3")
    ...
    p.add_argument("--chrom-subset-hv", action="store_true", default=True)
    p.add_argument("--no-chrom-subset-hv", action="store_false", dest="chrom_subset_hv")
    p.add_argument("--chrom-n-lsi", type=int, default=50)
    p.add_argument("--chrom-lsi-iter", type=int, default=15)
    p.add_argument("--chrom-lsi-backend", default="auto", choices=["auto", "scglue", "sklearn"])
    ...
    out_dir = (
        ...
        if args.out_dir
        else repo_root / f"integration/outputs/scglue/joint/{args.run_id}/preprocess"
    )
    ...
    for mark in marks:
        ...
        # Keep original genomic bin name and namespace by mark for joint graph/training.
    ...
    write_tsv(
        chrom_manifest_tsv,
        chrom_rows,
        ["mark", "chrom_h5ad", "n_cells", "n_features", "n_hv_features", "n_lsi", "clean_metadata_match_fraction"],
    )
