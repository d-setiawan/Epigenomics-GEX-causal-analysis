...
def save_scanpy_modality_facets(adata: ad.AnnData, out_path: Path) -> None:
    ...
    n_rows = math.ceil(max(1, n_panels) / n_cols)
...
def main() -> int:
    ...
    p.add_argument(
        "--transfer-labels",
        action="store_true",
        help="Transfer harmonized RNA labels to non-RNA modalities using kNN in Jianle space",
    )
    ...
    save_scanpy_umap(adata, color="modality_key", out_path=umap_mod_png, title="Joint Jianle UMAP by modality")
    ...
    save_scanpy_umap(adata, color="leiden", out_path=umap_cluster_png, title="Joint Jianle UMAP by Leiden cluster")
    ...
    if annotation_path and annotation_path.exists():
        ...
        plot_label_overlay(
            ...
            out_path=umap_celltype_png,
            title=f"Joint Jianle UMAP with RNA {label_level}",
        )
        ...
        if args.transfer_labels:
            ...
            plot_label_overlay(
                ...
                title="Joint Jianle UMAP with transferred harmonized coarse labels",
            )
            ...
            plot_label_overlay(
                ...
                title="Joint Jianle UMAP with transferred harmonized fine labels",
            )
